...
- `desktop/`: Wails desktop app with a separate entrypoint, backend methods for reading configured identities, active identity, effective identity state, desktop identity add/update/delete/activate operations, read-only diagnostic checks through existing core modules and config persistence, encrypted backup export/import through the existing core archive package, and app-native notifications/dialogs for user feedback and confirmations.
